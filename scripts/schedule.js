...
// Separate, namespaced classes for the card wrapper — reusing the
// status-* names here would collide with main.css's unscoped
// .status-free / .status-a / .status-b rules (background color for the
// pill), which are not scoped to .status-pill and would paint the whole
// card if the same class landed on .group-card too.
const SCHEDULE_LEVEL_CARD_CLASS = {
  'з нуля': 'level-free',
  A1: 'level-a1',
  A2: 'level-a2',
  B1: 'level-b1',
...
function scheduleFindTeacherLink(teacherName) {
  const teacher = scheduleAllTeachers.find(
    (t) =>
  ...
  grid.innerHTML = filtered
    .map((row) => {
      ...
      const badgeClass =
        ...
        'status-a';
      const cardClass =
        SCHEDULE_LEVEL_CARD_CLASS[scheduleNormalizeLevel(row.level)] ||
        'level-a1';
      ...
      const teacherLink = scheduleFindTeacherLink(row.teacher);
      ...
      return `
        <div class="group-card ${cardClass}">
          ...
          <ul class="group-rows">
            <li>
              ${dot}
            Розклад: <b>${row.schedule}</b></li>
            <li>${dot}Викладач: <b>${row.teacher}</b> ${teacherVideoIcon}</li>
            <li>${dot}Вартість: <b>${priceLine}</b></li>
            ${startLine}
          </ul>
